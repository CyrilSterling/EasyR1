...
from verl.utils.reward_score.gpt_as_judge import get_compare_messages, openai_llm
...
def test_vllm_judge(
    ...
    ground_truths = ["45 square units", "42 square units", "81"]
    ...
    print(
        "About to call openr1_compute_score_batch_vllm. Add breakpoint here if needed."
    )
    ...
    # Print results
    print("\nReward results:")
    for i, (pred, truth, reward) in enumerate(
        zip(predict_strs, ground_truths, rewards)
    ):
        print(f"\nExample {i+1}:")
...
if __name__ == "__main__":
    ...
    test_vllm_judge(
        base_url=args.base_url, model_name=args.model_name, api_key=args.api_key
    )
