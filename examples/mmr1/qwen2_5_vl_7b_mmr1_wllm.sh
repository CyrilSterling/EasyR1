...
RUNNING_JOBS=$(ray job list | grep RUNNING | grep "$JOB_NAME " | wc -l)
...
TRAIN_DATA=${TRAIN_DATA:-rl_pub_8k}

if [ -d /mnt/amlfs-01/home/jingwang/DATASET/mmo1/HF/$TRAIN_DATA/ ]; then
  TRAIN_DATA=/mnt/amlfs-01/home/jingwang/DATASET/mmo1/HF/$TRAIN_DATA/
else
  TRAIN_DATA=$TRAIN_DATA
fi

CONFIG=${CONFIG:-mmr1_batchvllm}
...
RAY_ADDRESS='http://127.0.0.1:8300' ray job submit \
  ...
  config=$WORKDIR/examples/${CONFIG}.yaml \
  data.system_prompt="${SYSTEM_PROMPT}" \
  data.train_files=$TRAIN_DATA \
  data.val_files=/mnt/amlfs-02/shared/ckpts/mmc/datasets--mm-o1--math_vista_val/snapshots/6d4db5f646fc7a5a4afe5599208870a929b38722 \
...
# OUTPUT_DIR=/mnt/amlfs-02/shared/jingwang/checkpoints/mmc/${JOB_NAME}

# FILENAME=$OUTPUT_DIR
# GLOBAL_STEP=$(ls $FILENAME/global_step_* | tail -n 1 | cut -d '_' -f 3)
# NAME=${NAME:-$(basename $FILENAME)}

# python EasyR1/scripts/model_merger.py --local_dir $FILENAME/global_step_$GLOBAL_STEP/actor/ --hf_upload_path mm-o1/$NAME